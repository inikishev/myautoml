...
def mimic_eigh(cov_ref: torch.Tensor, X_new: torch.Tensor, reg=1e-6):
...
class EighMimic(_BaseMimic):
    def __init__(self, include, exclude=None, device=CUDA_IF_AVAILABLE, dtype=torch.float64):
        self.include = include
        self.exclude = exclude
        self.device = device
        self.dtype = dtype
    ...
    def _fit(self, X_ref, X_new):
        ...
        W = mimic_eigh(cov_ref=cov_ref, X_new=X_new)
        self.W_ = W.numpy(force=True)
