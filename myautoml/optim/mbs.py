...
def _tofloatlist(x) -> list[float]:
    if isinstance(x, np.ScalarType): return [float(x)] # type:ignore
    if isinstance(x, np.ndarray) and x.size == 1: return [float(x.item())]

    if hasattr(x, "detach") and hasattr(x, "cpu") and x.numel() == 1: # pyright:ignore[reportAttributeAccessIssue]
        return [float(x.detach().cpu().item())] # pyright: ignore[reportAttributeAccessIssue]
...
class MBS:
    ...
    Args:
        grid: values for initial grid search. If ``log_scale=True``, should be in log10 scale.
        step: expansion step size. If ``log_scale=True``, should be in log10 scale. Defaults to 1.
        num_candidates: number of best points to sample new points around on each iteration. Defaults to 2.
        num_binary: maximum number of new points sampled via binary search. Defaults to 7.
        num_expansions: maximum number of expansions (not counted towards binary search points). Defaults to 7.
        rounding: rounding is to significant digits, avoids evaluating points that are too close.
        lb: lower bound. If ``log_scale=True``, should be in log10 scale.
        ub: upper bound. If ``log_scale=True``, should be in log10 scale.
        log_scale:
            whether to minimize in log10 scale. If true, it is assumed that
            ``grid``, ``step``, ``lb`` and ``ub`` are given in log10 scale.
